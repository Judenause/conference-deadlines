...
웹의 **Manage** 화면은 자체 관리 서버에서 관리자 ID/PW 로그인 후 학회 추가와 수기 일정 수정 요청을 저장합니다. 이 요청은 검수 전까지 공개 카탈로그를 바꾸지 않으며, 공개 데이터의 최종 원본은 계속 Git PR입니다. [자체 관리 서버 운영](docs/management-server-operations.md)을 따라 설정합니다.
